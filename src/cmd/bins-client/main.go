...
func kv(k, v string) *store.KeyValue {
	return &store.KeyValue{k, v}
}

func pat(pre, suf string) *store.Pattern {
	return &store.Pattern{pre, suf}
}

func kva(args []string) *store.KeyValue {
	if len(args) == 1 {
		return kv("", "")
	} else if len(args) == 2 {
		return kv(args[1], "")
...
func pata(args []string) *store.Pattern {
	if len(args) == 1 {
		return pat("", "")
	} else if len(args) == 2 {
		return pat(args[1], "")
...
func main() {
	...
	s := bin_client.NewBinClient(rc.Keepers, rc.Backs)
